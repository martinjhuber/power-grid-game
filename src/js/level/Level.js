...
import { LevelGenerator, snapshotPuzzle } from './LevelGenerator.js';
...
import { calculateScore } from '../score/ScoreCalculator.js';
...
export class Level {
  ...
  update(timePassed) {
    ...
    if (connectedTiles === this.#gridWidth * this.#gridHeight && !this.#won) {
      ...
      const verification = ReplayVerifier.verify(payload);
      if (CONFIG.debug) {
        console.debug('GameSession payload:', payload);
        console.debug('Replay verification:', verification);
      }
